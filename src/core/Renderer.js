...
export class Renderer {
    ...
    _paintBackdrop(ctx, w, h) {
        // Açık yeşil çiftlik gökyüzü atmosferi
        const sky = ctx.createLinearGradient(0, 0, 0, h);
        sky.addColorStop(0, 'rgba(210, 240, 188, 0.55)');
        sky.addColorStop(0.55, 'rgba(195, 228, 168, 0.0)');
        sky.addColorStop(1, 'rgba(170, 208, 140, 0.18)');
        ctx.fillStyle = sky;
        ctx.fillRect(0, 0, w, h);

        // Güneş parlaması: sıcak sarı-yeşil
        const bloom = ctx.createRadialGradient(
            w * 0.70, h * 0.18, 0,
            w * 0.70, h * 0.18, Math.max(w, h) * 0.85,
        );
        bloom.addColorStop(0, 'rgba(220, 248, 170, 0.45)');
        bloom.addColorStop(0.45, 'rgba(220, 248, 170, 0.08)');
        bloom.addColorStop(1, 'rgba(220, 248, 170, 0)');
        ctx.fillStyle = bloom;
        ctx.fillRect(0, 0, w, h);

        // Hafif doğa doku noktaları
        const cellSize = 24;
        const cx = w / 2, cy = h / 2;
        ...
        for (let x = 0; x < w; x += cellSize) {
            ...
            const a = 0.04 * (1 - r * 0.85);
            if (a <= 0) continue;
            ctx.fillStyle = `rgba(40, 80, 20, ${a.toFixed(3)})`;
